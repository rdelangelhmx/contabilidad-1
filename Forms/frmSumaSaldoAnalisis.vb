Public Class frmSumaSaldoAnalisis
...
#Region " Código generado por el Diseñador de Windows Forms "
   ...
   <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
      ...
      Dim resources As System.ComponentModel.ComponentResourceManager = New System.ComponentModel.ComponentResourceManager(GetType(frmSumaSaldoAnalisis))
...
#End Region
   ...
   Private Sub frmSumaSaldoAnalisis_Activated(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Activated
      Call StatusFormDisplay(Me.Name)
   End Sub

   Private Sub frmSumaSaldoAnalisis_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
      Dim oCursor As Cursor
      oCursor = Me.Cursor
      Me.Cursor = Cursors.WaitCursor
...
#Region " Print-Report "
   ...
   Private Sub PrintReport()
      ...
      If mlngMonedaId = 1 Then
         rpt.txtMoneda.Text = mstrMonedaDesBoliviano
      ElseIf mlngMonedaId = 2 Then
         rpt.txtMoneda.Text = mstrMonedaDesDolares
      End If
...
#End Region
   ...
   Private Sub frmSumaSaldoAnalisis_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
   ...
   Private Sub frmSumaSaldoAnalisis_Closed(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Closed
      moCompDet.Dispose()
      Call ClearMemory()
   End Sub
...
#Region " DataSet Suma-Saldo "
   Private Sub moDataSetInit()
      ...
      moDataTable.Columns.Add("PlanPadre1Des", Type.GetType("System.String"))
      moDataTable.Columns.Add("PlanPadre2Des", Type.GetType("System.String"))
      moDataTable.Columns.Add("PlanPadre3Des", Type.GetType("System.String"))
      moDataTable.Columns.Add("PlanPadre4Des", Type.GetType("System.String"))
      ...
      While datFechaIni < datFechaFin
         ...
         While datFechaIni < datFechaFinGestion
            moDataTable.Columns.Add("GralBs" & lngPos, Type.GetType("System.Decimal"))
            moDataTable.Columns.Add("GralUs" & lngPos, Type.GetType("System.Decimal"))
            datFechaIni = datFechaIni.AddMonths(1)
            datFechaIni = DateSerial(Year(datFechaIni), Month(datFechaIni), 1)
            lngPos += 1
         End While

         moDataTable.Columns.Add("Ajuste" & lngGestion, Type.GetType("System.Decimal"))
         moDataTable.Columns.Add("SaldoAcu" & lngGestion, Type.GetType("System.Decimal"))
      End While
   ...
   Private Function GetRow(ByVal lngPlanId As Long, ByRef orow As DataRow) As DataRow
      With moPlan
         ...
         Dim lngMonedaId As Long = ToLong(rowPlan("MonedaId"))

         If lngMonedaId = clsMoneda.BOLIVIANOS Then
            orow("MonedaDes") = mstrMonedaDesBoliviano
         ElseIf lngMonedaId = clsMoneda.DOLARES Then
            orow("MonedaDes") = mstrMonedaDesDolares
         End If

         orow("PlanPadre1Des") = ToStr((From r In .DataSet.Tables(.TableName) Where r("PlanCta") = strPlanCta.Substring(0, 1) Select r("PlanDes")).FirstOrDefault)
         orow("PlanPadre2Des") = ToStr((From r In .DataSet.Tables(.TableName) Where r("PlanCta") = strPlanCta.Substring(0, 2) Select r("PlanDes")).FirstOrDefault)
         orow("PlanPadre3Des") = ToStr((From r In .DataSet.Tables(.TableName) Where r("PlanCta") = strPlanCta.Substring(0, 3) Select r("PlanDes")).FirstOrDefault)
         orow("PlanPadre4Des") = ToStr((From r In .DataSet.Tables(.TableName) Where r("PlanCta") = strPlanCta.Substring(0, 4) Select r("PlanDes")).FirstOrDefault)
      End With
   ...
   Private Function GetFooter(ByVal lngMonedaId As Long) As DataRow
      ...
      If lngMonedaId = 1 Then
         oRow("PlanDes") = "Total Saldos en " & mstrMonedaDesBoliviano
      ElseIf lngMonedaId = 2 Then
         oRow("PlanDes") = "Total Saldos en " & mstrMonedaDesDolares
      End If

      oRow("SaldoAnt") = ToDecimal(moDataTable.Compute("SUM(SaldoAnt)", ""))
      ...
      While datFechaIni < datFechaFin
         ...
         While datFechaIni < datFechaFinGestion
            oRow("GralBs" & lngPos) = ToDecimal(moDataTable.Compute("SUM(GralBs" & lngPos & ")", ""))
            oRow("GralUs" & lngPos) = ToDecimal(moDataTable.Compute("SUM(GralUs" & lngPos & ")", ""))
            datFechaIni = datFechaIni.AddMonths(1)
            datFechaIni = DateSerial(Year(datFechaIni), Month(datFechaIni), 1)
            lngPos += 1
         End While

         oRow("SaldoAcu" & lngGestion) = ToDecimal(moDataTable.Compute("SUM(SaldoAcu" & lngGestion & ")", ""))
         oRow("Ajuste" & lngGestion) = ToDecimal(moDataTable.Compute("SUM(Ajuste" & lngGestion & ")", ""))
      End While
...
#Region " Grid Suma-Saldo "
   ...
   Private Sub grdMainLoad()
      ...
      Dim decSaldoGestionBs As Decimal = 0
      Dim decSaldoGestionUs As Decimal = 0
      Dim decSaldoGestionBsAux As Decimal = 0
      Dim decSaldoGestionUsAux As Decimal = 0
      ...
      Try
         ...
         For Each lngPlanId As Long In moLista
            ...
            While datFechaIni < datFechaFin
               ...
               mstrTipoAsientoId = TipoAsientoIdListaLoad(boolFirst)
               ...
               If boolFirst Then
                  datFechaIniGestion = DateSerial(datFechaIni.Year - 1, datFechaIni.Month, datFechaIni.Day)
                  Dim lngGestionIdAux = GestionIdFind(datFechaIniGestion, 0)
                  mstrTipoAsientoId = TipoAsientoIdListaLoad(lngGestionIdAux = 0)
                  decSaldoBs = SaldoGestionBs(lngPlanId, datFechaIniGestion, DateSerial(datFechaIni.Year, datFechaIni.Month, datFechaIni.Day - 1), decSaldoUs)

                  If mlngGrupo = 3 Or mlngGrupo = 2 Or mlngGrupo = 4 Then 'PATRIMONIO, PASIVO y INGRESO
                     decSaldoBs *= -1
                     decSaldoUs *= -1
                  End If

                  If mlngMonedaId = clsMoneda.BOLIVIANOS Then
                     oRow("SaldoAnt") = decSaldoBs
                  Else
                     oRow("SaldoAnt") = decSaldoUs
                  End If

                  If mlngGrupo <> 4 And mlngGrupo <> 5 And mlngGrupo <> 6 Then
                     decSaldoGestionBs += decSaldoBs
                     decSaldoGestionUs += decSaldoUs
                  End If

                  mdecSaldoPlanBs += decSaldoBs
                  mdecSaldoPlanUs += decSaldoUs

                  If ToDecimal(oRow("SaldoAnt")) = 0 Then
                     oRow("SaldoAnt") = System.Convert.DBNull
                  End If
               ...
               Dim datFechaFinTemp As Date = DateSerial(datFechaIni.Year, datFechaIni.Month, datFechaIni.DaysInMonth(datFechaIni.Year, datFechaIni.Month))

               datFechaIniAux = datFechaIni

               If datatableCompDet IsNot Nothing Then
                  While datFechaIni < datFechaFinGestion
                     Dim strFiltro As String = "Fecha >= '" & ToDateDMY(datFechaIni) & "' and Fecha <= '" & ToDateDMY(datFechaFinTemp) & "'"
                     ...
                     oRow("GralBs" & lngPos) = decSaldoBs
                     oRow("GralUs" & lngPos) = decSaldoUs

                     If ToDecimal(oRow("GralBs" & lngPos)) = 0 Then
                        oRow("GralBs" & lngPos) = System.Convert.DBNull
                     End If

                     If ToDecimal(oRow("GralUs" & lngPos)) = 0 Then
                        oRow("GralUs" & lngPos) = System.Convert.DBNull
                     End If
                     ...
                     If boolActivoFijo And mlngGestionIdFinal = mlngGestionId Then
                        decSaldoActivoBs = SaldoActivoFijoCompraBajaBs(datatableCompDetActivoFijo, datFechaIni, datFechaFinTemp, decSaldoActivoUs)

                        oRow("GralBs" & lngPos) = decSaldoActivoBs
                        oRow("GralUs" & lngPos) = decSaldoActivoUs
                     ...
                     mdecSaldoPlanBs += decSaldoBs
                     mdecSaldoPlanUs += decSaldoUs
                     decSaldoGestionBsAux += decSaldoBs - decSaldoActivoBs
                     decSaldoGestionUsAux += decSaldoUs - decSaldoActivoUs

                     datFechaIni = datFechaIni.AddMonths(1)
                     datFechaIni = DateSerial(Year(datFechaIni), Month(datFechaIni), 1)
                     datFechaFinTemp = DateSerial(Year(datFechaIni), Month(datFechaIni), datFechaIni.DaysInMonth(datFechaIni.Year, datFechaIni.Month))
                     lngPos += 1
                  ...
                  If mlngGrupo = 3 Or lngPlanId = clsAppInfo.PlanAitbId Or boolActivoFijo Then
                     If mlngMonedaId = clsMoneda.BOLIVIANOS Then
                        oRow("Ajuste" & lngGestion) = decSaldoGestionBsAux
                     ElseIf mlngMonedaId = clsMoneda.DOLARES Then
                        oRow("Ajuste" & lngGestion) = decSaldoGestionUsAux
                     End If
                  End If

               Else
                  Dim lngCant = DateDiff(DateInterval.Month, datFechaIni, datFechaFinGestion) + 1
                  datFechaIni = DateSerial(Year(datFechaFinGestion), Month(datFechaFinGestion), datFechaIni.DaysInMonth(datFechaFinGestion.Year, datFechaFinGestion.Month) + 1)
                  lngPos += lngCant
               End If

               If mlngMonedaId = clsMoneda.BOLIVIANOS Then
                  oRow("SaldoAcu" & lngGestion) = decSaldoGestionBs
               ElseIf mlngMonedaId = clsMoneda.DOLARES Then
                  oRow("SaldoAcu" & lngGestion) = decSaldoGestionUs
               End If

               'If mlngGestionIdFinal = mlngGestionId And lngPlanId = clsAppInfo.PlanAitbId Then
               '   If mlngMonedaId = clsMoneda.BOLIVIANOS Then
               '      oRow("SaldoAcu" & lngGestion) = decSaldoGestionBsAux
               '   ElseIf mlngMonedaId = clsMoneda.DOLARES Then
               '      oRow("SaldoAcu" & lngGestion) = decSaldoGestionUsAux
               '   End If
               'End If

               decSaldoGestionBsAux = 0
               decSaldoGestionUsAux = 0
   ...
   Private Sub grdMainInit()
      ...
      With grdMain
         ...
         .RootTable.Columns("PlanPadre1Des").Caption = "A/P/PN"
         .RootTable.Columns("PlanPadre1Des").Width = 80
         .RootTable.Columns("PlanPadre1Des").CellStyle.TextAlignment = Janus.Windows.GridEX.TextAlignment.Near
         .RootTable.Columns("PlanPadre1Des").HeaderStyle.TextAlignment = Janus.Windows.GridEX.TextAlignment.Center

         .RootTable.Columns("PlanPadre2Des").Caption = "AC/ANC/PC/PNC/PN"
         .RootTable.Columns("PlanPadre2Des").Width = 150
         .RootTable.Columns("PlanPadre2Des").CellStyle.TextAlignment = Janus.Windows.GridEX.TextAlignment.Near
         .RootTable.Columns("PlanPadre2Des").HeaderStyle.TextAlignment = Janus.Windows.GridEX.TextAlignment.Center

         .RootTable.Columns("PlanPadre3Des").Caption = "Rubro Según Balance"
         .RootTable.Columns("PlanPadre3Des").Width = 150
         .RootTable.Columns("PlanPadre3Des").CellStyle.TextAlignment = Janus.Windows.GridEX.TextAlignment.Near
         .RootTable.Columns("PlanPadre3Des").HeaderStyle.TextAlignment = Janus.Windows.GridEX.TextAlignment.Center

         .RootTable.Columns("PlanPadre4Des").Caption = "Subrubro Según Balance"
         .RootTable.Columns("PlanPadre4Des").Width = 150
         .RootTable.Columns("PlanPadre4Des").CellStyle.TextAlignment = Janus.Windows.GridEX.TextAlignment.Near
         .RootTable.Columns("PlanPadre4Des").HeaderStyle.TextAlignment = Janus.Windows.GridEX.TextAlignment.Center
         ...
         .RootTable.Columns("SaldoAnt").Caption = "Saldo Anterior"
         ...
         .RootTable.Columns("SaldoAnt").CellStyle.BackColor = Color.Lavender
         ...
         While datFechaIni < datFechaFin
            ...
            While datFechaIni < datFechaFinGestion
               ...
               .RootTable.Columns("GralBs" & lngPos).Caption = strMes & " " & datFechaIni.Year
               .RootTable.Columns("GralBs" & lngPos).FormatString = DecimalMask()
               .RootTable.Columns("GralBs" & lngPos).Width = 80
               .RootTable.Columns("GralBs" & lngPos).CellStyle.TextAlignment = Janus.Windows.GridEX.TextAlignment.Far
               .RootTable.Columns("GralBs" & lngPos).HeaderStyle.TextAlignment = Janus.Windows.GridEX.TextAlignment.Center

               .RootTable.Columns("GralUs" & lngPos).Caption = strMes & " " & datFechaIni.Year
               .RootTable.Columns("GralUs" & lngPos).FormatString = DecimalMask()
               .RootTable.Columns("GralUs" & lngPos).Width = 80
               .RootTable.Columns("GralUs" & lngPos).CellStyle.TextAlignment = Janus.Windows.GridEX.TextAlignment.Far
               .RootTable.Columns("GralUs" & lngPos).HeaderStyle.TextAlignment = Janus.Windows.GridEX.TextAlignment.Center

               If mlngMonedaId = clsMoneda.BOLIVIANOS Then
                  .RootTable.Columns("GralUs" & lngPos).Visible = False
               ElseIf mlngMonedaId = clsMoneda.DOLARES Then
                  .RootTable.Columns("GralBs" & lngPos).Visible = False
               End If
            ...
            .RootTable.Columns("Ajuste" & lngGestion).Caption = "Ajuste por Inflación " & datFechaIniAux.ToString("MMM/yyyy") & " a " & datFechaFinGestion.ToString("MMM/yyyy")
            .RootTable.Columns("Ajuste" & lngGestion).FormatString = DecimalMask()
            .RootTable.Columns("Ajuste" & lngGestion).Width = 100
            .RootTable.Columns("Ajuste" & lngGestion).CellStyle.TextAlignment = Janus.Windows.GridEX.TextAlignment.Far
            .RootTable.Columns("Ajuste" & lngGestion).HeaderStyle.TextAlignment = Janus.Windows.GridEX.TextAlignment.Center
            .RootTable.Columns("Ajuste" & lngGestion).CellStyle.BackColor = Color.MistyRose

            .RootTable.Columns("SaldoAcu" & lngGestion).Caption = "Saldo Acumulado al " & ToDateDMY(datFechaFinGestion)
            .RootTable.Columns("SaldoAcu" & lngGestion).FormatString = DecimalMask()
            .RootTable.Columns("SaldoAcu" & lngGestion).Width = 100
            .RootTable.Columns("SaldoAcu" & lngGestion).CellStyle.TextAlignment = Janus.Windows.GridEX.TextAlignment.Far
            .RootTable.Columns("SaldoAcu" & lngGestion).HeaderStyle.TextAlignment = Janus.Windows.GridEX.TextAlignment.Center
            ' .RootTable.Columns("SaldoAcu" & lngGestion).CellStyle.BackColor = Color.YellowGreen
            .RootTable.Columns("SaldoAcu" & lngGestion).CellStyle.BackColor = Color.LightGreen
         End While
      End With
