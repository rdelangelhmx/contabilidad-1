Public Class frmTipoCambio
   ...
   Friend WithEvents NewRange As Janus.Windows.UI.CommandBars.UICommand
   Friend WithEvents Import As Janus.Windows.UI.CommandBars.UICommand
   Friend WithEvents NewRange1 As Janus.Windows.UI.CommandBars.UICommand
...
#Region " Código generado por el Diseñador de Windows Forms "
   ...
   <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
      ...
      Me.NewRange1 = New Janus.Windows.UI.CommandBars.UICommand("NewRange")
      ...
      Me.NewRange = New Janus.Windows.UI.CommandBars.UICommand("NewRange")
      ...
      Me.ilsMain.Images.SetKeyName(8, "page_excel.png")
      '
      'cdmMain
      '
      ...
      Me.cdmMain.Commands.AddRange(New Janus.Windows.UI.CommandBars.UICommand() {Me.New2, Me.Edit, Me.Delete, Me.ViewAll, Me.Print, Me.Exit2, Me.Refrescar, Me.NewRange, Me.Import})
      ...
      Me.UiCommandBar1.Commands.AddRange(New Janus.Windows.UI.CommandBars.UICommand() {Me.New1, Me.Separator1, Me.NewRange1, Me.Separator7, Me.Import1, Me.Separator4, Me.Edit1, Me.Separator2, Me.Delete1, Me.Separator3, Me.Refrescar1, Me.Separator6, Me.ViewAll1, Me.Separator8, Me.Print1, Me.Separator5, Me.Exit1})
      ...
      'NewRange1
      '
      Me.NewRange1.Key = "NewRange"
      Me.NewRange1.Name = "NewRange1"
      '
      'Separator7
      '
      ...
      'NewRange
      '
      Me.NewRange.ImageIndex = 0
      Me.NewRange.Key = "NewRange"
      Me.NewRange.Name = "NewRange"
      Me.NewRange.Text = "Nuevo por Rango"
      '
      'Import
      '
      Me.Import.ImageIndex = 7
...
#End Region
   ...
   Private Sub frmTipoCambio_Activated(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Activated
   ...
   Private Sub grdMainLoad()
      Try
         With moTipoCambio
            .SelectFilter = clsTipoCambio.SelectFilters.Grid
            .OrderByFilter = clsTipoCambio.OrderByFilters.Grid
   ...
   Private Sub DataShow()
      ...
      Try
         If (grdMain.GetRow.Selected) And (ToLong(grdMain.GetValue("TipoCambioId")) > 0) Then
            With oTipoCambio
               .TipoCambioId = ToLong(grdMain.GetValue("TipoCambioId"))
   ...
   Private Sub FormNewRange()
      Dim frm As New frmTipoCambioRangeEdit
      Dim oTipoCambio As New clsTipoCambio(clsAppInfo.ConnectString)
   ...
   Private Sub ImportData()
      Dim frmImportExcel As New frmImportarTipoCambioFind
      Try
         ...
         With frmImportExcel
            ...
            If .Changed Then
               ...
               If ImportCargar(.DataObject, .Opcion) Then
                  MessageBox.Show("Se Importaron los datos exitosamente.", Me.Text, MessageBoxButtons.OK, MessageBoxIcon.Asterisk)
               End If
            End If
   ...
   Private Function ImportCargar(ByVal objDataSet As DataSet, ByVal lngOpcion As Long) As Boolean
      Dim oTipoCambio As New clsTipoCambio(clsAppInfo.ConnectString)

      Try

         For Each oRow As DataRow In objDataSet.Tables(0).Rows
            If Not String.IsNullOrEmpty(ToStr(oRow(0))) Then
               If Not IsDBNull(oRow(0)) Then
                  Dim strFecha As String = ToStr(oRow(0))
                  ...
                  With oTipoCambio
                     ...
                     If Not .Find Then
                        Select Case lngOpcion
                           Case 1
                              .Valor = ToDecimal(oRow(1))
                              .Ufv = ToDecimalCinco(oRow(2))
                           Case 2
                              .Valor = ToDecimal(oRow(1))
                           Case 3
                              .Ufv = ToDecimalCinco(oRow(1))
                        End Select
         ...
         If clsAppInfo.RegistrarBitacora Then
            Dim table As DataTable = objDataSet.Tables(0)
            Dim strFechaIni As String = ToStr((From row In table Select row.Field(Of Date)(0)).Min)
            Dim strFechaFin As String = ToStr((From row In table Select row.Field(Of Date)(0)).Max)

            Call modAuditoria.AudiOpeAdd(Me.Name, Me.Text, clsTipoOpera.IMPORTAR, "Importar: " & If(lngOpcion = 1, "Ambos", If(lngOpcion = 2, "Tipo de Cambio", "UFV")) & "; Desde: " & strFechaIni & "; Hasta: " & strFechaFin)
         End If
   ...
   Private Sub CloneVM(ByRef oVM As clsTipoCambioVM)
      If clsAppInfo.RegistrarBitacora Then
         ...
         With oVM
            ...
            .Valor = ToStr(grdMain.GetValue("Valor"))
            .UFV = ToStr(grdMain.GetValue("UFV"))
      ...
      Try
         If grdMain.RowCount > 0 Then
            If MessageBox.Show("¿Realmente quiere eliminar el registro?", Me.Text, MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
               With oTipoCambio
                  .WhereFilter = clsTipoCambio.WhereFilters.PrimaryKey
                  .TipoCambioId = ToLong(grdMain.GetValue("TipoCambioId"))
   ...
   Private Sub PrintReport()
      Dim rpt As New rptTipoCambio

      rpt.DataSource = GetDataView(moTipoCambio.DataSet.Tables(moTipoCambio.TableName))

      rpt.lblTitle.Text = "Tipos de Cambio"
      rpt.Document.Name = "Tipos de Cambio"
   ...
   Private Sub grdMainFindRow(ByVal lngID As Long)
      ...
      oCol = grdMain.RootTable.Columns("TipoCambioId")
   ...
   Private Sub FormInit()
      ...
      moTipoCambio = New clsTipoCambio(clsAppInfo.ConnectString)
   End Sub

   Private Sub grdMainInit()
      With grdMain
         ebrMain.Groups(0).Text = "Tipos de Cambio"
         .ContextMenu = mnuMain

         .RootTable.Columns("TipoCambioId").Visible = False

         .RootTable.Columns("Fecha").Caption = "Fecha"
         .RootTable.Columns("Fecha").FormatString = "dd/MM/yyyy, dddd"
         .RootTable.Columns("Fecha").Width = 150
         ...
         .RootTable.Columns("Valor").Caption = "Tipo Cambio"
         .RootTable.Columns("Valor").FormatString = DecimalMask()
         .RootTable.Columns("Valor").Width = 100
         .RootTable.Columns("Valor").CellStyle.TextAlignment = Janus.Windows.GridEX.TextAlignment.Far
         .RootTable.Columns("Valor").HeaderStyle.TextAlignment = Janus.Windows.GridEX.TextAlignment.Center

         .RootTable.Columns("Ufv").Caption = "U.F.V."
         .RootTable.Columns("Ufv").FormatString = "##,##0.00000"
         .RootTable.Columns("Ufv").Width = 100
         .RootTable.Columns("Ufv").CellStyle.TextAlignment = Janus.Windows.GridEX.TextAlignment.Far
         .RootTable.Columns("Ufv").HeaderStyle.TextAlignment = Janus.Windows.GridEX.TextAlignment.Center
      End With
   End Sub
   ...
   Private Sub frmTipoCambio_Closed(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Closed
