Public Class frmMovFlujoEfectivo
...
#Region " Código generado por el Diseñador de Windows Forms "
   ...
   <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
      ...
      Dim resources As System.ComponentModel.ComponentResourceManager = New System.ComponentModel.ComponentResourceManager(GetType(frmMovFlujoEfectivo))
      ...
      Me.KeyPreview = True
      Me.Name = "frmMovFlujoEfectivo"
      Me.StartPosition = System.Windows.Forms.FormStartPosition.CenterScreen
      Me.Text = "Movimiento de Flujos de Efectivo"
...
#End Region
   ...
   Private Sub frmMovFlujoEfectivo_Activated(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Activated
      Call StatusFormDisplay(Me.Name)
   End Sub

   Private Sub frmMovFlujoEfectivo_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
      Dim oCursor As Cursor
      oCursor = Me.Cursor
      Me.Cursor = Cursors.WaitCursor
   ...
   Private Sub PrintReport()
      Try
         If (mlngMonedaId = 1) Or (mlngMonedaId = 2) Then
            Dim rpt As New rptFlujoEfectivo
            rpt.DataSource = GetDataView(moDataTable.DataSet.Tables(moCompPlanEEFF.TableName))

            rpt.lblTitle.Text = "Movimiento de Flujos de Efectivo"
            rpt.Document.Name = "Movimiento de Flujos de Efectivo"
         ...
         ElseIf (mlngMonedaId = 3) Or (mlngMonedaId = 4) Then
            Dim rpt As New rptFlujoEfectivoExt
   ...
   Private Sub frmMovFlujoEfectivo_Closed(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Closed
      moCompPlanEEFF.Dispose()
      Call ClearMemory()
   End Sub

   Private Sub frmMovFlujoEfectivo_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
...
#Region " Metodo-FLUJO EFECTIVO "
   ...
   Private Sub AddConditionalFormatting()
      ...
      fc = New Janus.Windows.GridEX.GridEXFormatCondition(grdMain.RootTable.Columns("CompPlanEEFFId"), Janus.Windows.GridEX.ConditionOperator.Equal, -1)
      fc.FormatStyle.ForeColor = Color.DarkGreen
      grdMain.RootTable.FormatConditions.Add(fc)

      fc = New Janus.Windows.GridEX.GridEXFormatCondition(grdMain.RootTable.Columns("CompPlanEEFFId"), Janus.Windows.GridEX.ConditionOperator.Equal, -3)
      fc.FormatStyle.ForeColor = Color.DarkGreen
      grdMain.RootTable.FormatConditions.Add(fc)
...
#Region " DataSet-FLUJO EFECTIVO "
   Private Sub moDataSetInit()
      ...
      moDataTable.Columns.Add("CompPlanEEFFId", Type.GetType("System.Int32"))
   ...
   Private Function GetRow(ByVal oDataRow As DataRow) As DataRow
      ...
      oRow("CompPlanEEFFId") = oDataRow("CompPlanEEFFId")
      ...
      oRow("PlanEEFFCta") = oDataRow("CompNro")

      If mboolCuenta Then
         oRow("PlanEEFFDes") = oDataRow("CompDetDes")
      Else
         oRow("PlanEEFFDes") = oDataRow("EntregadoA")
      End If
      ...
      oRow("SaldoBs") = oDataRow("DebeBs") - oDataRow("HaberBs")
      oRow("DebeUs") = oDataRow("DebeUs")
      oRow("HaberUs") = oDataRow("HaberUs")
      oRow("SaldoUs") = oDataRow("DebeUs") - oDataRow("HaberUs")
   ...
   Private Function GetFooter(ByVal lngPlanEEFFId As Long, ByVal lngId As Long) As DataRow 'ByVal lngMonedaId As Long) As DataRow
      Dim oRow As DataRow
      oRow = moDataTable.NewRow
      ...
      If lngId = -3 Then ' Total Grupo
         decDebeBs = moCompPlanEEFF.DataSet.Tables(moCompPlanEEFF.TableName).Compute("SUM(DebeBs)", "PlanPadreEEFFId = " & ToStr(lngPlanEEFFId))
         decDebeUs = moCompPlanEEFF.DataSet.Tables(moCompPlanEEFF.TableName).Compute("SUM(DebeUs)", "PlanPadreEEFFId = " & ToStr(lngPlanEEFFId))
         decHaberBs = moCompPlanEEFF.DataSet.Tables(moCompPlanEEFF.TableName).Compute("SUM(HaberBs)", "PlanPadreEEFFId = " & ToStr(lngPlanEEFFId))
         decHaberUs = moCompPlanEEFF.DataSet.Tables(moCompPlanEEFF.TableName).Compute("SUM(HaberUs)", "PlanPadreEEFFId = " & ToStr(lngPlanEEFFId))

         strDes = (From row In moCompPlanEEFF.DataSet.Tables(moCompPlanEEFF.TableName) Where row.Field(Of Integer)("PlanPadreEEFFId") = lngPlanEEFFId Select row.Field(Of String)("PlanPadreEEFFDes")).First
         oRow("PlanEEFFDes") = "Total Gral. " & strDes

      ElseIf lngId = -4 Then
         decDebeBs = moCompPlanEEFF.DataSet.Tables(moCompPlanEEFF.TableName).Compute("SUM(DebeBs)", "PlanEEFFId = " & ToStr(lngPlanEEFFId))
         decDebeUs = moCompPlanEEFF.DataSet.Tables(moCompPlanEEFF.TableName).Compute("SUM(DebeUs)", "PlanEEFFId = " & ToStr(lngPlanEEFFId))
         decHaberBs = moCompPlanEEFF.DataSet.Tables(moCompPlanEEFF.TableName).Compute("SUM(HaberBs)", "PlanEEFFId = " & ToStr(lngPlanEEFFId))
         decHaberUs = moCompPlanEEFF.DataSet.Tables(moCompPlanEEFF.TableName).Compute("SUM(HaberUs)", "PlanEEFFId = " & ToStr(lngPlanEEFFId))

         strDes = (From row In moCompPlanEEFF.DataSet.Tables(moCompPlanEEFF.TableName) Where row.Field(Of Integer)("PlanEEFFId") = lngPlanEEFFId Select row.Field(Of String)("PlanEEFFDes")).First
         oRow("PlanEEFFDes") = "Total " & strDes
      End If
...
#Region " Grid-FLUJO EFECTIVO "
   Private Sub grdMainLoad()
      ...
      Try
         With moCompPlanEEFF
            ...
            .CompPlanEEFFId = If(mboolIntervaloFecha, 1, 0)
            .Fecha = mstrFechaIni
            ...
            If .Open() And .DataSet.Tables(.TableName).Rows.Count > 0 Then
               ...
               Dim lngPlanPadreEEFFId As Long = -1
               Dim lngPlanEEFFId As Long = -1

               For Each oRow As DataRow In .DataSet.Tables(.TableName).Rows
                  If (lngPlanPadreEEFFId = -1 Or lngPlanPadreEEFFId <> ToLong(oRow("PlanPadreEEFFId"))) Or (lngPlanEEFFId = -1 Or lngPlanEEFFId <> ToLong(oRow("PlanEEFFId"))) Then

                     If lngPlanPadreEEFFId <> -1 And lngPlanEEFFId <> -1 Then
                        If lngPlanEEFFId <> ToLong(oRow("PlanEEFFId")) Then
                           moDataTable.Rows.Add(GetFooter(lngPlanEEFFId, -4))

                           If lngPlanPadreEEFFId <> ToLong(oRow("PlanPadreEEFFId")) Then
                              moDataTable.Rows.Add(GetFooter(lngPlanPadreEEFFId, -3))
                           End If
                     ...
                     If lngPlanPadreEEFFId = -1 Or lngPlanPadreEEFFId <> ToLong(oRow("PlanPadreEEFFId")) Then
                        moDataTable.Rows.Add(GetHeader(oRow, -1))
                     End If

                     If lngPlanEEFFId = -1 Or lngPlanEEFFId <> ToLong(oRow("PlanEEFFId")) Then
                        moDataTable.Rows.Add(GetHeader(oRow, -2))
                     End If

                     lngPlanEEFFId = ToLong(oRow("PlanEEFFId"))
                     lngPlanPadreEEFFId = ToLong(oRow("PlanPadreEEFFId"))
   ...
   Private Sub grdMainInit()
      With grdMain
         ...
         .RootTable.Columns("CompPlanEEFFId").Visible = False
         ...
         .RootTable.Columns("PlanEEFFCta").Caption = "Nro"
         .RootTable.Columns("PlanEEFFCta").Width = 80
         .RootTable.Columns("PlanEEFFCta").CellStyle.TextAlignment = Janus.Windows.GridEX.TextAlignment.Center
         .RootTable.Columns("PlanEEFFCta").HeaderStyle.TextAlignment = Janus.Windows.GridEX.TextAlignment.Center

         .RootTable.Columns("PlanEEFFDes").Caption = "Glosa"
